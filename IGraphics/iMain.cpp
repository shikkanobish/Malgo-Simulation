...
void iDraw()
{
	...
	//iFilledCircle(x,y,r);
	//iText(250,250,"CUBE(125)",GLUT_BITMAP_HELVETICA_24);
	if(title)
	{
	...
	if(eqn3)
	{
		...
		if(meqnsolflag)
		{
			if(einvalid)
			{
				iText(364-strlen("INVALID!")*14,371,"INVALID!",GLUT_BITMAP_TIMES_ROMAN_24);
			}
			else 
			{
				sprintf(meqnsol, "%0.3lf", ex);
...
int main()
{
	...
	iInitialize(1000, 708, "Malgo Simulation");
	//int title=1;
	for(int i=0;i<8;i++)mpre[i]=0;
	for(int i=0;i<5;i++)mpost[i]=0;
	return 0;
